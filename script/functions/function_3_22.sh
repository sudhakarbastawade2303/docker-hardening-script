...
if [[ -f "$SYS_CONFIG_DOCKER_PATH" ]]; then
    ...
    if [[ "$current_owner" == "$desired_owner" ]]; then
        echo "The ownership of $SYS_CONFIG_DOCKER_PATH is correctly set to $desired_owner."
    else
        echo "NOTE: The ownership of $SYS_CONFIG_DOCKER_PATH is incorrect. Current ownership is $current_owner."
