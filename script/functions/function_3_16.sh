...
if [[ -e "$DOCKER_SOCKET_PATH" ]]; then
    ...
    if [[ "$current_permissions" -le "$desired_permissions" ]]; then
        echo "The permissions of $DOCKER_SOCKET_PATH are correctly set to $current_permissions or more restrictive."
    else
        echo "NOTE: The permissions of $DOCKER_SOCKET_PATH are too permissive. Current permissions are $current_permissions."
