...
check_live_restore_config_file() {
    ...
    local live_restore_option='"live-restore": true'
    ...
    if [[ -f "$config_file" ]]; then
        ...
        if jq -e ".[\"live-restore\"] == true" "$config_file" > /dev/null; then
            echo "PASS: Live restore is enabled in $config_file."
            return 0
        else
            echo "FAIL: Live restore is NOT enabled in $config_file."
            echo "NOTE: You need to add the following line to $config_file:"
            echo "$live_restore_option"
