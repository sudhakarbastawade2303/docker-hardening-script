...
ensure_audit_containerd_shim_runc_v2() {
    ...
    if grep -q -- "$rule_containerd_shim_runc_v2" "$audit_rules_file"; then
        echo "PASS: Audit rule for /usr/bin/containerd-shim-runc-v2 is already configured."
        return 0
    else
        echo "FAIL: Audit rule for /usr/bin/containerd-shim-runc-v2 is not configured."
