...
if [[ -f "$DOCKER_SERVER_CERT_PATH" ]]; then
    ...
    if [[ "$current_owner" == "$desired_owner" ]]; then
        echo "The ownership of $DOCKER_SERVER_CERT_PATH is correctly set to $desired_owner."
    else
        echo "NOTE: The ownership of $DOCKER_SERVER_CERT_PATH is incorrect. Current ownership is $current_owner."
